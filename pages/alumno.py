...
st.set_page_config(page_title="TutorIA · Alumno", page_icon="🎓", layout="wide")
inject_css()
require_auth()
...
<div class="metric-row">
    <div class="metric-box">
        ...
        <div class="lbl">Total tutorías</div>
    </div>
    <div class="metric-box">
...
# ── COLUMNA IZQUIERDA: Disponibilidad + Agendar ──────────
with col_left:

    # Disponibilidad de docentes
    st.markdown("""
    <div class="tutoria-card">
        <h3>📅 Disponibilidad de tutores</h3>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not slots:
        ...
        for s in slots:
            doc = s.get("perfiles", {})
            nombre_doc = f"{doc.get('nombre','')} {doc.get('apellido','')}"
            fecha  = s["fecha"]
    ...
    if not slots_libres:
        ...
        # Construir opciones: "Nombre · Fecha · Hora"
        opciones = {}
        ...
        descripcion = st.text_area("Descripción (opcional)", placeholder="Describe brevemente tu duda…",
                                   height=80)
        ...
        if st.button("✅ Solicitar tutoría", type="primary", use_container_width=True):
            if not materia:
                ...
                if ok:
                    st.markdown('<div class="msg-success">✅ ¡Tutoría agendada correctamente! Revisa tu historial.</div>',
                                unsafe_allow_html=True)
                    st.rerun()
...
with col_right:
    ...
    if programadas_list:
        ...
        opciones_cancel = {
            f"{datetime.fromisoformat(s['fecha_hora'].replace('Z','')).strftime('%d/%m/%Y %H:%M')} · {s.get('materia','—')}": s
            for s in programadas_list
        }
        sel_cancel = st.selectbox("Selecciona la sesión a cancelar", list(opciones_cancel.keys()),
                                  key="cancel_sel")
